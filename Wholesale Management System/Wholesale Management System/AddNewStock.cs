...
namespace Wholesale_Management_System
{
    public partial class AddNewStock : Form
    {
        ...
        {
            populateGoodsComboBox("SELECT goodID,goodName FROM Goods WHERE (goodID NOT IN  (SELECT  goodID FROM  Stocks))");
        }
        ...
        private void addStockButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                databaseSingleton.sql = "INSERT INTO Stocks (goodID , quantity) VALUES ('" + goodID + "','" + quantity + "')";
                ...
                //////
                databaseSingleton.sql = "SELECT TOP 1 stockID FROM Stocks ORDER BY stockID DESC";
                databaseSingleton.connection = new SqlConnection(databaseSingleton.connetionString);
                databaseSingleton.connection.Open();
                databaseSingleton.command = new SqlCommand(databaseSingleton.sql, databaseSingleton.connection);
